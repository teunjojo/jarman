...
#######################################
# Updates a JAR to the latest version via SpigotMC
# Globals:
#	None
# Arguments:
#   jar_json
...
#######################################
# Returns the latest version of a JAR via SpigotMC
# Globals:
#	None
# Arguments:
#   jar_json
#######################################
spigotmc_get_version() {
	local jar_json=$1
	local jar_loader=$(echo "$jar_json" | jq -r '.loader')
	[ -z "$jar_loader" ] && error_handler "JAR loader not set"
	local jar_project=$(echo "$jar_json" | jq -r '.project')
	...
	local spigotmc_latest_version=""
	[ -z "$spigotmc_latest_version" ] && error_handler "Failed to retrieve latest version number"

	echo "$spigotmc_latest_version"
}
